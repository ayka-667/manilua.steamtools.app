...
export async function getAdminUsageRows() {
  ...
  const [rows] = await pool.query(
    `SELECT user_id, day_start_ms, daily_count, daily_limit, cooldown_until_ms, updated_at_ms
  ...
  return rows.map((row) => {
    ...
    const rowDayStartMs = Number(row.day_start_ms) || dayStartMs;
    return {
      userId: row.user_id,
      ...
      downloadsRemaining: Math.max(dailyLimit - used, 0),
      cooldownSec: cooldownUntilMs > nowMs ? Math.ceil((cooldownUntilMs - nowMs) / 1000) : 0,
      dayResetAt: rowDayStartMs + DAY_MS,
