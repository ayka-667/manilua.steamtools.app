...
function resetKeyForUser(userId) {
  return `premium_cooldown_reset:${userId}`;
}

export async function getCooldownResetAt(userId) {
  const pool = await getMySqlPool();
  const [rows] = await pool.query(
    "SELECT setting_value FROM app_settings WHERE setting_key = ?",
    [resetKeyForUser(userId)]
  );
  const value = Number(rows?.[0]?.setting_value || 0);
  return Number.isNaN(value) ? 0 : value;
}

export async function setCooldownResetAt(userId) {
  const pool = await getMySqlPool();
  const now = Date.now();
  await pool.query(
    `INSERT INTO app_settings (setting_key, setting_value, updated_at_ms)
     VALUES (?, ?, ?)
     ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value), updated_at_ms = VALUES(updated_at_ms)`,
    [resetKeyForUser(userId), String(now), now]
  );
  return now;
}
...
export async function updatePremiumOrderStatus({
  ...
  const pool = await getMySqlPool();
  const now = Date.now();
  if (status === "reset_cooldown") {
    await pool.query(
      `UPDATE premium_orders SET updated_at_ms = ?, payment_note = ? WHERE id = ?`,
      [now, note || null, id]
    );
    return;
  }
