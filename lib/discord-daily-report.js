...
const REPORT_CHANNEL_ID = "1487572491307192461";
...
async function fetchChannelMessages(botToken, before = "") {
  const url = new URL(`https://discord.com/api/v10/channels/${REPORT_CHANNEL_ID}/messages`);
...
async function deleteChannelMessage(botToken, messageId) {
  const response = await fetch(`https://discord.com/api/v10/channels/${REPORT_CHANNEL_ID}/messages/${messageId}`, {
    method: "DELETE",
    headers: {
      Authorization: `Bot ${botToken}`
    },
...
async function sendDiscordDailyRecap({ content, files }) {
  ...
  const response = await fetch(`https://discord.com/api/v10/channels/${REPORT_CHANNEL_ID}/messages`, {
    method: "POST",
    headers: {
      Authorization: `Bot ${botToken}`
    },
...
export async function runDailyDiscordReport({ force = false } = {}) {
  ...
  if (!force && parisNow.hour !== 0) {
    return {
      ok: true,
      skipped: true,
      reason: "not_midnight_in_paris"
    };
  }

  const reportDate = shiftDateParts(parisNow, -1);
