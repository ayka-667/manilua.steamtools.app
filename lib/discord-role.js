...
export async function checkPremiumRole(userId) {
  ...
  return hasRole
    ? { allowed: true, reason: "" }
    : { allowed: false, reason: "Premium required. Visit /premium to upgrade." };
}
...
export async function addRoleToMember(userId, roleId) {
  const guildId = process.env.DISCORD_GUILD_ID;
  const botToken = process.env.DISCORD_BOT_TOKEN;

  if (!guildId || !botToken || !userId || !roleId) {
    return { ok: false, status: 0, error: "Discord role grant misconfigured." };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { ok: false, status: response.status, error: errorText || "Discord role grant failed." };
    }

    return { ok: true, status: response.status, error: "" };
  } catch {
    return { ok: false, status: 0, error: "Discord role grant failed." };
  }
}
